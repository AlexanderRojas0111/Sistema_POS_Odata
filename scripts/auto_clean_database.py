#!/usr/bin/env python3
"""
Auto Clean Database - Mantener Solo 18 Productos Sabrositas
==========================================================
...
def clean_database(db_path: str):
    ...
    try:
        ...
        if existing_product_ids:
            ...
            print("⚠️  No se encontraron productos Sabrositas existentes")
            deleted_products = 0
...
def verify_sabrositas_products(db_path: str):
    """Verificar que los productos Sabrositas estén presentes"""
    try:
        conn = sqlite3.connect(db_path)
        cursor = conn.cursor()
        ...
        for i, product in enumerate(products, 1):
            ...
            print(f"{i:2d}. {name:<15} | SKU: {sku:<10} | ${price:>6} | Stock: {stock:>3}")
        
        # Verificar productos faltantes
        missing_products = [name for name in sabrositas_names if name not in found_names]
        if missing_products:
            print(f"\n⚠️  PRODUCTOS SABROSITAS FALTANTES ({len(missing_products)}):")
            for product in missing_products:
                print(f"   - {product}")
        else:
            print(f"\n✅ PRODUCTOS SABROSITAS ENCONTRADOS: {len([n for n in found_names if n in sabrositas_names])}")
        
        conn.close()
        return len(products), len(missing_products)
...
def main():
    """Función principal"""
    print("🧹 AUTO-LIMPIEZA DE BASE DE DATOS SABROSITAS")
    print("=" * 50)
    ...
    try:
        ...
        total_products, missing_count = verify_sabrositas_products(db_path)
        ...
        print(f"⚠️  Productos faltantes: {missing_count}")
